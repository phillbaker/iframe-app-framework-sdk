...
function rawPostMessage(client, msg, forceReady) {
  if (client.ready || forceReady) {
    // console.log('client sending', msg, 'to', client._origin);
...
function getMessageRecipient(client, recipientId) {
  var messageRecipient = client;

  if (recipientId && recipientId !== client._instanceId) {
    messageRecipient = client._instanceClients[recipientId];

    if (!messageRecipient) {
      throw Error('[IAF SDK] Could not find client for instance ' + recipientId);
    }
  }
...
function messageHandler(client, event) {
  ...
  var clientRecipient = getMessageRecipient(client, data.instanceId),
      pendingPromise;

  if (data.id && (pendingPromise = pendingPromises[data.id])) {
    finalizePendingPromise(pendingPromise, data);
  } else if (IAF_EVENT.test(data.key)) {
    var key = data.key.replace(IAF_EVENT, ''),
        msg = { appId: client._appId };
    ...
    // console.log('client fell through', data);
  }
}
...
var Client = function(options) {
  ...
  this._appId = options.appId || this._parent && this._parent._appId;
  this._instanceId = options.instanceId || this._appId;
...
Client.prototype = {
  ...
  ///
  /// Allows you to send message events to the host app.
  ///
  /// ##### Arguments
  ///
  ...
  ///
  /// ```javascript
  /// var client = IAFClient.init();
  /// client.postMessage('hello', { awesome: true });
  /// ```
  postMessage: function(name, data) {
    var msg = JSON.stringify({ key: name, message: data, appId: this._appId, instanceId: this._instanceId });
  ...
  /// ```
  ///
  /// Note: As soon as communication with the host app is established, the SDK triggers an
  ...
  ///
  /// Dispatch [requests](./requests) via the host app.
  ///
  /// ##### Arguments
  ///
  ...
  request: function(options) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      this.on(requestKey + '.done', function(evt) {
        resolve.call(this, evt.responseArgs);
      });

      this.on(requestKey + '.fail', function(evt) {
        reject.call(this, evt.responseArgs);
      });
  ...
  instance: function(instanceId) {
    if (!instanceId || typeof instanceId !== 'string') {
      throw new Error('The instance method expects an `instanceId` string.');
    }
    if (instanceId === this._instanceId) { return this; }
    if (this._parent) { return this._parent.instance(instanceId); }
    var instanceClient = this._instanceClients[instanceId];
    if (!instanceClient) {
      instanceClient = new Client({
        parent: this,
        instanceId: instanceId
      });
      this._instanceClients[instanceId] = instanceClient;
    }
  ...
  context: function() {
    if (this._context) {
      ...
      if (this._instanceId && this._instanceId != this._appId) {
        var key = 'instances.' + this._instanceId;
